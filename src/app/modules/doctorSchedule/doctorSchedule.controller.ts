...
export const getMySchedules = catchAsync(async (req, res) => {
  const user = req.user
  const { startDate, endDate, ...filterData } = pick(req.query,["startDate","endDate","isBooked"]);
  const andCondition = [];
...
export const deleteMySchedules = catchAsync(async (req, res) => {
  ...
  if(isBookedSchedule){
    throw new ApiError(httpCode.BAD_REQUEST,'can not delete the booked schedule')
  }
